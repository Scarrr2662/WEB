    ...
    <div class="max-w-3xl mx-auto py-12 px-4 sm:py-8 lg:py-16">
        ...
        <div class="flex justify-between items-center mb-5">
            <div>
                <h3 class="text-lg font-semibold text-gray-800">Data Suplier</h3>
            ...
            <table class="min-w-full bg-white">
                <thead>
                    <tr>
                        ...
                        <th class="px-4 py-3 border-b border-gray-200 bg-gray-50 text-gray-500 text-left text-sm font-normal">Name</th>
                        <th class="px-4 py-3 border-b border-gray-200 bg-gray-50 text-gray-500 text-left text-sm font-normal">Address</th>
                        <th class="px-4 py-3 border-b border-gray-200 bg-gray-50 text-gray-500 text-left text-sm font-normal">Phone Number</th>
                ...
                <tbody>
                    ...
                    <tr class="hover:bg-gray-50 border-b border-gray-200">
                        ...
                        <td class="px-4 py-4 text-sm text-gray-500">{{ $supplier->nama_supplier }}</td>
                        <td class="px-4 py-4 text-sm text-gray-500">{{ $supplier->alamat_supplier }}</td>
                        <td class="px-4 py-4 text-sm text-gray-500">{{ $supplier->telepon_supplier }}</td>
                        <td class="px-4 py-4">
                            <a href="{{ route('suplier.edit', $supplier->id_supplier) }}" class="text-blue-600 hover:text-blue-800 font-medium">Edit</a>
                        </td>
                        <td class="px-4 py-4">
                            <a href="{{ route('suplier.destroy', $supplier->id_supplier) }}"
                            onclick="event.preventDefault(); if(confirm('Are you sure you want to delete this supplier?')) { document.getElementById('delete-form-{{ $supplier->id_supplier }}').submit(); }"
                            class="text-red-600 hover:text-red-800 font-medium">Delete</a>
                            <form id="delete-form-{{ $supplier->id_supplier }}" action="{{ route('suplier.destroy', $supplier->id_supplier) }}" method="POST" style="display: none;">
                                @csrf
                                @method('DELETE')
                            </form>
                        </td>
        ...
        <div class="flex justify-center mt-6">
            <a href="{{ route('suplier.create') }}" class="bg-blue-500 hover:bg-blue-400 text-white font-bold py-2 px-4 border-b-4 border-blue-700 hover:border-blue-500 rounded">
