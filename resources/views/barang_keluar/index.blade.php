...
<div class="max-w-3xl mx-auto py-12 px-4 sm:py-8 lg:py-16">
    ...
    <div class="flex justify-between items-center mb-5">
        <div>
            <h3 class="text-lg font-semibold text-gray-800">Data Barang Keluar</h3>
            <p class="text-gray-500">A list of all the items that have been shipped out.</p>
        </div>
        <div class="w-full max-w-xs relative">
            <input
                ...
                placeholder="Search for item data..."
            />
        ...
        <table class="min-w-full bg-white">
            <thead>
                <tr>
                    ...
                    <th class="px-4 py-3 border-b border-gray-200 bg-gray-50 text-gray-500 text-left text-sm font-normal">Tanggal Keluar</th>
                    <th class="px-4 py-3 border-b border-gray-200 bg-gray-50 text-gray-500 text-left text-sm font-normal">Jumlah Keluar</th>
                    <th class="px-4 py-3 border-b border-gray-200 bg-gray-50 text-gray-500 text-left text-sm font-normal">Lokasi</th>
                    <th class="px-4 py-3 border-b border-gray-200 bg-gray-50 text-gray-500 text-left text-sm font-normal">Penerima</th>
            ...
            <tbody>
                ...
                <tr class="hover:bg-gray-50 border-b border-gray-200">
                    ...
                    <td class="px-4 py-4 text-sm text-gray-500">{{ $barang->id_barang }}</td>
                    <td class="px-4 py-4 text-sm text-gray-500">{{ $barang->nama_barang }}</td>
                    <td class="px-4 py-4 text-sm text-gray-500">{{ date('d-m-Y', strtotime($barang->tgl_keluar)) }}</td>
                    <td class="px-4 py-4 text-sm text-gray-500">{{ $barang->jml_keluar }}</td>
                    <td class="px-4 py-4 text-sm text-gray-500">{{ $barang->lokasi }}</td>
                    <td class="px-4 py-4 text-sm text-gray-500">{{ $barang->penerima }}</td>
                    <td class="px-4 py-4">
                        <a href="{{ route('barang_keluar.edit', $barang->id) }}" class="text-blue-600 hover:text-blue-800 font-medium">Edit</a>
                    </td>
                    <td class="px-4 py-4">
                        <a href="{{ route('barang_keluar.destroy', $barang->id) }}"
                        ...
                        <form id="delete-form-{{ $barang->id }}" action="{{ route('barang_keluar.destroy', $barang->id) }}" method="POST" style="display: none;">
                            @csrf
                            @method('DELETE')
                        </form>
                    </td>
