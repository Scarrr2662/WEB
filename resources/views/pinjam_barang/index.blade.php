...
@section('content')
<!-- Component -->
<div class="max-w-5xl mx-auto py-12 px-4 sm:py-8 lg:py-16">

    <!-- Header Section -->
    <div class="flex justify-between items-center mb-5">
        <div>
            <h3 class="text-lg font-semibold text-gray-800">Data Pinjam Barang</h3>
            <p class="text-gray-500">List of all goods loan data.</p>
        </div>
        ...
        <table class="min-w-full bg-white">
            <thead>
                <tr>
                    ...
                    <th class="px-4 py-3 border-b border-gray-200 bg-gray-50 text-gray-500 text-left text-sm font-normal">Id Pinjam</th>
                    <th class="px-4 py-3 border-b border-gray-200 bg-gray-50 text-gray-500 text-left text-sm font-normal">Peminjam</th>
                    <th class="px-4 py-3 border-b border-gray-200 bg-gray-50 text-gray-500 text-left text-sm font-normal">Tanggal Pinjam</th>
                    ...
                    <th class="px-4 py-3 border-b border-gray-200 bg-gray-50 text-gray-500 text-left text-sm font-normal">Jumlah</th>
                    <th class="px-4 py-3 border-b border-gray-200 bg-gray-50 text-gray-500 text-left text-sm font-normal">Tanggal Kembali</th>
                    <th class="px-4 py-3 border-b border-gray-200 bg-gray-50 text-gray-500 text-left text-sm font-normal">Kondisi</th>
            ...
            <tbody>
                ...
                <tr class="hover:bg-gray-50 border-b border-gray-200">
                    ...
                    <td class="px-4 py-4 text-sm text-gray-500">{{ $item->id_pinjam }}</td>
                    <td class="px-4 py-4 text-sm text-gray-500">{{ $item->peminjam }}</td>
                    <td class="px-4 py-4 text-sm text-gray-500">{{ $item->tgl_pinjam }}</td>
                    <td class="px-4 py-4 text-sm text-gray-500">{{ $item->id_barang }}</td>
                    <td class="px-4 py-4 text-sm text-gray-500">{{ $item->nama_barang }}</td>
                    <td class="px-4 py-4 text-sm text-gray-500">{{ $item->jml_barang }}</td>
                    <td class="px-4 py-4 text-sm text-gray-500">{{ $item->tgl_kembali }}</td>
                    <td class="px-4 py-4 text-sm text-gray-500">{{ $item->kondisi }}</td>
                    <td class="px-4 py-4">
                        <a href="{{ route('pinjam_barang.edit', $item->id) }}" class="text-blue-600 hover:text-blue-800 font-medium">Edit</a>
                    </td>
                    <td class="px-4 py-4">
                        <a href="{{ route('pinjam_barang.destroy', $item->id) }}"
                        ...
                        <form id="delete-form-{{ $item->id }}" action="{{ route('pinjam_barang.destroy', $item->id) }}" method="POST" style="display: none;">
                            @csrf
                            @method('DELETE')
                        </form>
                    </td>
    ...
    <div class="flex justify-center mt-6">
        <a href="{{ route('pinjam_barang.create') }}" class="bg-blue-500 hover:bg-blue-400 text-white font-bold py-2 px-4 border-b-4 border-blue-700 hover:border-blue-500 rounded">
